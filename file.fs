...
let primeSequence =
    1L
    |> Seq.unfold(fun a -> Some(a, a + 1L))
    |> Seq.filter isPrime

let naturalSequence = 
    (1L,1L) |> Seq.unfold(fun (i,nat) -> Some((nat),(i+1L,nat + i + 1L)))

let primeFactors (i:int64):int64 list = 
    let rec primeFactors_inner (i:int64) (itr:int64) (acc:int64 list):int64 list = 
        if isPrime i then 
            acc @ [i] 
        else 
            if itr > i then 
                acc 
            elif i % itr = 0 then // Is divisible
                primeFactors_inner (i / itr) itr (acc @ [itr])        
            else
                primeFactors_inner  i (itr + 1L) (acc)   
    primeFactors_inner i 2L []


let factorCount (i:int64) = 
    primeFactors i
    |> List.countBy id
    |> List.map snd
    |> List.map(fun a -> a + 1)
...
(1,1) 
|> Seq.unfold(fun (i,nat) -> Some(nat,(i+1,nat + i + 1))   
)
|> Seq.filter(fun a -> factorCount a > 500)
|> Seq.take 1 
|> printfn "%A"
